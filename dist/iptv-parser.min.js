/**!
 * @license IPTV-Parser.js - A pure vanilla JavaScript library for EASILY parsing M3U / IPTV
 * VERSION: 1.0.0
 * LICENSED UNDER MIT LICENSE
 * MORE INFO CAN BE FOUND AT https://github.com/MarketingPipeline/IPTV-Parser.js/
 */ import*as A from"https://cdnjs.cloudflare.com/polyfill/v3/polyfill.min.js?version=4.8.0features=Array.prototype.map,fetch,Promise";var e={742:()=>{!function(){if("undefined"!=typeof document&&!("adoptedStyleSheets"in document)){var A="ShadyCSS"in window&&!ShadyCSS.nativeShadow,e=document.implementation.createHTMLDocument(""),t=new WeakMap,n="object"==typeof DOMException?Error:DOMException,r=Object.defineProperty,o=Array.prototype.forEach,a=/@import.+?;?$/gm,i=CSSStyleSheet.prototype;i.replace=function(){return Promise.reject(new n("Can't call replace on non-constructed CSSStyleSheets."))},i.replaceSync=function(){throw new n("Failed to execute 'replaceSync' on 'CSSStyleSheet': Can't call replaceSync on non-constructed CSSStyleSheets.")};var s=new WeakMap,c=new WeakMap,l=new WeakMap,u=new WeakMap,f=M.prototype;f.replace=function(A){try{return this.replaceSync(A),Promise.resolve(this)}catch(A){return Promise.reject(A)}},f.replaceSync=function(A){if(O(this),"string"==typeof A){var e=this;s.get(e).textContent=function(A){var e=A.replace(a,"");return e!==A&&console.warn("@import rules are not allowed here. See https://github.com/WICG/construct-stylesheets/issues/119#issuecomment-588352418"),e.trim()}(A),u.set(e,[]),c.get(e).forEach((function(A){A.isConnected()&&z(e,g(e,A))}))}},r(f,"cssRules",{configurable:!0,enumerable:!0,get:function(){return O(this),s.get(this).sheet.cssRules}}),r(f,"media",{configurable:!0,enumerable:!0,get:function(){return O(this),s.get(this).sheet.media}}),["addRule","deleteRule","insertRule","removeRule"].forEach((function(A){f[A]=function(){var e=this;O(e);var t=arguments;u.get(e).push({method:A,args:t}),c.get(e).forEach((function(n){if(n.isConnected()){var r=g(e,n).sheet;r[A].apply(r,t)}}));var n=s.get(e).sheet;return n[A].apply(n,t)}})),r(M,Symbol.hasInstance,{configurable:!0,value:w});var d={childList:!0,subtree:!0},p=new WeakMap,m=new WeakMap,h=new WeakMap,v=new WeakMap;if(X.prototype={isConnected:function(){var A=m.get(this);return A instanceof Document?"loading"!==A.readyState:function(A){return"isConnected"in A?A.isConnected:document.contains(A)}(A.host)},connect:function(){var A=T(this);v.get(this).observe(A,d),h.get(this).length>0&&J(this),W(A,(function(A){P(A).connect()}))},disconnect:function(){v.get(this).disconnect()},update:function(A){var e=this,t=m.get(e)===document?"Document":"ShadowRoot";if(!Array.isArray(A))throw new TypeError("Failed to set the 'adoptedStyleSheets' property on "+t+": Iterator getter is not callable.");if(!A.every(w))throw new TypeError("Failed to set the 'adoptedStyleSheets' property on "+t+": Failed to convert value to 'CSSStyleSheet'");if(A.some(E))throw new TypeError("Failed to set the 'adoptedStyleSheets' property on "+t+": Can't adopt non-constructed stylesheets");e.sheets=A;var n,r,o=h.get(e),a=(n=A).filter((function(A,e){return n.indexOf(A)===e}));(r=a,o.filter((function(A){return-1===r.indexOf(A)}))).forEach((function(A){var t;(t=g(A,e)).parentNode.removeChild(t),function(A,e){l.get(A).delete(e),c.set(A,c.get(A).filter((function(A){return A!==e})))}(A,e)})),h.set(e,a),e.isConnected()&&a.length>0&&J(e)}},window.CSSStyleSheet=M,R(Document),"ShadowRoot"in window){R(ShadowRoot);var b=Element.prototype,y=b.attachShadow;b.attachShadow=function(A){var e=y.call(this,A);return"closed"===A.mode&&t.set(this,e),e}}var x=P(document);x.isConnected()?x.connect():document.addEventListener("DOMContentLoaded",x.connect.bind(x))}function V(A){return A.shadowRoot||t.get(A)}function w(A){return"object"==typeof A&&(f.isPrototypeOf(A)||i.isPrototypeOf(A))}function E(A){return"object"==typeof A&&i.isPrototypeOf(A)}function g(A,e){return l.get(A).get(e)}function z(A,e){requestAnimationFrame((function(){e.textContent=s.get(A).textContent,u.get(A).forEach((function(A){return e.sheet[A.method].apply(e.sheet,A.args)}))}))}function O(A){if(!s.has(A))throw new TypeError("Illegal invocation")}function M(){var A=this,t=document.createElement("style");e.body.appendChild(t),s.set(A,t),c.set(A,[]),l.set(A,new WeakMap),u.set(A,[])}function P(A){var e=p.get(A);return e||(e=new X(A),p.set(A,e)),e}function R(A){r(A.prototype,"adoptedStyleSheets",{configurable:!0,enumerable:!0,get:function(){return P(this).sheets},set:function(A){P(this).update(A)}})}function W(A,e){for(var t=document.createNodeIterator(A,NodeFilter.SHOW_ELEMENT,(function(A){return V(A)?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT}),null,!1),n=void 0;n=t.nextNode();)e(V(n))}function T(A){var e=m.get(A);return e instanceof Document?e.body:e}function J(A){var e=document.createDocumentFragment(),t=h.get(A),n=v.get(A),r=T(A);n.disconnect(),t.forEach((function(t){e.appendChild(g(t,A)||function(A,e){var t=document.createElement("style");return l.get(A).set(e,t),c.get(A).push(e),t}(t,A))})),r.insertBefore(e,null),n.observe(r,d),t.forEach((function(e){z(e,g(e,A))}))}function X(e){var t=this;t.sheets=[],m.set(t,e),h.set(t,[]),v.set(t,new MutationObserver((function(e,n){document?e.forEach((function(e){A||o.call(e.addedNodes,(function(A){A instanceof Element&&W(A,(function(A){P(A).connect()}))})),o.call(e.removedNodes,(function(e){e instanceof Element&&(function(A,e){return e instanceof HTMLStyleElement&&h.get(A).some((function(e){return g(e,A)}))}(t,e)&&J(t),A||W(e,(function(A){P(A).disconnect()})))}))})):n.disconnect()})))}}()}},t={};function n(A){var r=t[A];if(void 0!==r)return r.exports;var o=t[A]={exports:{}};return e[A](o,o.exports,n),o.exports}n.d=(A,e)=>{for(var t in e)n.o(e,t)&&!n.o(A,t)&&Object.defineProperty(A,t,{enumerable:!0,get:e[t]})},n.o=(A,e)=>Object.prototype.hasOwnProperty.call(A,e);(A=>{var e={};n.d(e,A)})({});n(742);function r(A){return r="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(A){return typeof A}:function(A){return A&&"function"==typeof Symbol&&A.constructor===Symbol&&A!==Symbol.prototype?"symbol":typeof A},r(A)}function o(){/*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/facebook/regenerator/blob/main/LICENSE */o=function(){return e};var A,e={},t=Object.prototype,n=t.hasOwnProperty,a=Object.defineProperty||function(A,e,t){A[e]=t.value},i="function"==typeof Symbol?Symbol:{},s=i.iterator||"@@iterator",c=i.asyncIterator||"@@asyncIterator",l=i.toStringTag||"@@toStringTag";function u(A,e,t){return Object.defineProperty(A,e,{value:t,enumerable:!0,configurable:!0,writable:!0}),A[e]}try{u({},"")}catch(A){u=function(A,e,t){return A[e]=t}}function f(A,e,t,n){var r=e&&e.prototype instanceof y?e:y,o=Object.create(r.prototype),i=new J(n||[]);return a(o,"_invoke",{value:P(A,t,i)}),o}function d(A,e,t){try{return{type:"normal",arg:A.call(e,t)}}catch(A){return{type:"throw",arg:A}}}e.wrap=f;var p="suspendedStart",m="suspendedYield",h="executing",v="completed",b={};function y(){}function x(){}function V(){}var w={};u(w,s,(function(){return this}));var E=Object.getPrototypeOf,g=E&&E(E(X([])));g&&g!==t&&n.call(g,s)&&(w=g);var z=V.prototype=y.prototype=Object.create(w);function O(A){["next","throw","return"].forEach((function(e){u(A,e,(function(A){return this._invoke(e,A)}))}))}function M(A,e){function t(o,a,i,s){var c=d(A[o],A,a);if("throw"!==c.type){var l=c.arg,u=l.value;return u&&"object"==r(u)&&n.call(u,"__await")?e.resolve(u.__await).then((function(A){t("next",A,i,s)}),(function(A){t("throw",A,i,s)})):e.resolve(u).then((function(A){l.value=A,i(l)}),(function(A){return t("throw",A,i,s)}))}s(c.arg)}var o;a(this,"_invoke",{value:function(A,n){function r(){return new e((function(e,r){t(A,n,e,r)}))}return o=o?o.then(r,r):r()}})}function P(e,t,n){var r=p;return function(o,a){if(r===h)throw Error("Generator is already running");if(r===v){if("throw"===o)throw a;return{value:A,done:!0}}for(n.method=o,n.arg=a;;){var i=n.delegate;if(i){var s=R(i,n);if(s){if(s===b)continue;return s}}if("next"===n.method)n.sent=n._sent=n.arg;else if("throw"===n.method){if(r===p)throw r=v,n.arg;n.dispatchException(n.arg)}else"return"===n.method&&n.abrupt("return",n.arg);r=h;var c=d(e,t,n);if("normal"===c.type){if(r=n.done?v:m,c.arg===b)continue;return{value:c.arg,done:n.done}}"throw"===c.type&&(r=v,n.method="throw",n.arg=c.arg)}}}function R(e,t){var n=t.method,r=e.iterator[n];if(r===A)return t.delegate=null,"throw"===n&&e.iterator.return&&(t.method="return",t.arg=A,R(e,t),"throw"===t.method)||"return"!==n&&(t.method="throw",t.arg=new TypeError("The iterator does not provide a '"+n+"' method")),b;var o=d(r,e.iterator,t.arg);if("throw"===o.type)return t.method="throw",t.arg=o.arg,t.delegate=null,b;var a=o.arg;return a?a.done?(t[e.resultName]=a.value,t.next=e.nextLoc,"return"!==t.method&&(t.method="next",t.arg=A),t.delegate=null,b):a:(t.method="throw",t.arg=new TypeError("iterator result is not an object"),t.delegate=null,b)}function W(A){var e={tryLoc:A[0]};1 in A&&(e.catchLoc=A[1]),2 in A&&(e.finallyLoc=A[2],e.afterLoc=A[3]),this.tryEntries.push(e)}function T(A){var e=A.completion||{};e.type="normal",delete e.arg,A.completion=e}function J(A){this.tryEntries=[{tryLoc:"root"}],A.forEach(W,this),this.reset(!0)}function X(e){if(e||""===e){var t=e[s];if(t)return t.call(e);if("function"==typeof e.next)return e;if(!isNaN(e.length)){var o=-1,a=function t(){for(;++o<e.length;)if(n.call(e,o))return t.value=e[o],t.done=!1,t;return t.value=A,t.done=!0,t};return a.next=a}}throw new TypeError(r(e)+" is not iterable")}return x.prototype=V,a(z,"constructor",{value:V,configurable:!0}),a(V,"constructor",{value:x,configurable:!0}),x.displayName=u(V,l,"GeneratorFunction"),e.isGeneratorFunction=function(A){var e="function"==typeof A&&A.constructor;return!!e&&(e===x||"GeneratorFunction"===(e.displayName||e.name))},e.mark=function(A){return Object.setPrototypeOf?Object.setPrototypeOf(A,V):(A.__proto__=V,u(A,l,"GeneratorFunction")),A.prototype=Object.create(z),A},e.awrap=function(A){return{__await:A}},O(M.prototype),u(M.prototype,c,(function(){return this})),e.AsyncIterator=M,e.async=function(A,t,n,r,o){void 0===o&&(o=Promise);var a=new M(f(A,t,n,r),o);return e.isGeneratorFunction(t)?a:a.next().then((function(A){return A.done?A.value:a.next()}))},O(z),u(z,l,"Generator"),u(z,s,(function(){return this})),u(z,"toString",(function(){return"[object Generator]"})),e.keys=function(A){var e=Object(A),t=[];for(var n in e)t.push(n);return t.reverse(),function A(){for(;t.length;){var n=t.pop();if(n in e)return A.value=n,A.done=!1,A}return A.done=!0,A}},e.values=X,J.prototype={constructor:J,reset:function(e){if(this.prev=0,this.next=0,this.sent=this._sent=A,this.done=!1,this.delegate=null,this.method="next",this.arg=A,this.tryEntries.forEach(T),!e)for(var t in this)"t"===t.charAt(0)&&n.call(this,t)&&!isNaN(+t.slice(1))&&(this[t]=A)},stop:function(){this.done=!0;var A=this.tryEntries[0].completion;if("throw"===A.type)throw A.arg;return this.rval},dispatchException:function(e){if(this.done)throw e;var t=this;function r(n,r){return i.type="throw",i.arg=e,t.next=n,r&&(t.method="next",t.arg=A),!!r}for(var o=this.tryEntries.length-1;o>=0;--o){var a=this.tryEntries[o],i=a.completion;if("root"===a.tryLoc)return r("end");if(a.tryLoc<=this.prev){var s=n.call(a,"catchLoc"),c=n.call(a,"finallyLoc");if(s&&c){if(this.prev<a.catchLoc)return r(a.catchLoc,!0);if(this.prev<a.finallyLoc)return r(a.finallyLoc)}else if(s){if(this.prev<a.catchLoc)return r(a.catchLoc,!0)}else{if(!c)throw Error("try statement without catch or finally");if(this.prev<a.finallyLoc)return r(a.finallyLoc)}}}},abrupt:function(A,e){for(var t=this.tryEntries.length-1;t>=0;--t){var r=this.tryEntries[t];if(r.tryLoc<=this.prev&&n.call(r,"finallyLoc")&&this.prev<r.finallyLoc){var o=r;break}}o&&("break"===A||"continue"===A)&&o.tryLoc<=e&&e<=o.finallyLoc&&(o=null);var a=o?o.completion:{};return a.type=A,a.arg=e,o?(this.method="next",this.next=o.finallyLoc,b):this.complete(a)},complete:function(A,e){if("throw"===A.type)throw A.arg;return"break"===A.type||"continue"===A.type?this.next=A.arg:"return"===A.type?(this.rval=this.arg=A.arg,this.method="return",this.next="end"):"normal"===A.type&&e&&(this.next=e),b},finish:function(A){for(var e=this.tryEntries.length-1;e>=0;--e){var t=this.tryEntries[e];if(t.finallyLoc===A)return this.complete(t.completion,t.afterLoc),T(t),b}},catch:function(A){for(var e=this.tryEntries.length-1;e>=0;--e){var t=this.tryEntries[e];if(t.tryLoc===A){var n=t.completion;if("throw"===n.type){var r=n.arg;T(t)}return r}}throw Error("illegal catch attempt")},delegateYield:function(e,t,n){return this.delegate={iterator:X(e),resultName:t,nextLoc:n},"next"===this.method&&(this.arg=A),b}},e}function a(A,e,t,n,r,o,a){try{var i=A[o](a),s=i.value}catch(A){return void t(A)}i.done?e(s):Promise.resolve(s).then(n,r)}function i(A){return function(){var e=this,t=arguments;return new Promise((function(n,r){var o=A.apply(e,t);function i(A){a(o,n,r,i,s,"next",A)}function s(A){a(o,n,r,i,s,"throw",A)}i(void 0)}))}}function s(){s=function(A,e){return new t(A,void 0,e)};var A=RegExp.prototype,e=new WeakMap;function t(A,n,r){var o=RegExp(A,n);return e.set(o,r||e.get(A)),m(o,t.prototype)}function n(A,t){var n=e.get(t);return Object.keys(n).reduce((function(e,t){var r=n[t];if("number"==typeof r)e[t]=A[r];else{for(var o=0;void 0===A[r[o]]&&o+1<r.length;)o++;e[t]=A[r[o]]}return e}),Object.create(null))}return f(t,RegExp),t.prototype.exec=function(e){var t=A.exec.call(this,e);if(t){t.groups=n(t,this);var r=t.indices;r&&(r.groups=n(r,this))}return t},t.prototype[Symbol.replace]=function(t,o){if("string"==typeof o){var a=e.get(this);return A[Symbol.replace].call(this,t,o.replace(/\$<([^>]+)>/g,(function(A,e){var t=a[e];return"$"+(Array.isArray(t)?t.join("$"):t)})))}if("function"==typeof o){var i=this;return A[Symbol.replace].call(this,t,(function(){var A=arguments;return"object"!=r(A[A.length-1])&&(A=[].slice.call(A)).push(n(A,i)),o.apply(this,A)}))}return A[Symbol.replace].call(this,t,o)},s.apply(this,arguments)}function c(A,e){for(var t=0;t<e.length;t++){var n=e[t];n.enumerable=n.enumerable||!1,n.configurable=!0,"value"in n&&(n.writable=!0),Object.defineProperty(A,l(n.key),n)}}function l(A){var e=function(A,e){if("object"!=r(A)||!A)return A;var t=A[Symbol.toPrimitive];if(void 0!==t){var n=t.call(A,e||"default");if("object"!=r(n))return n;throw new TypeError("@@toPrimitive must return a primitive value.")}return("string"===e?String:Number)(A)}(A,"string");return"symbol"==r(e)?e:e+""}function u(A,e){if(e&&("object"==r(e)||"function"==typeof e))return e;if(void 0!==e)throw new TypeError("Derived constructors may only return object or undefined");return function(A){if(void 0===A)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return A}(A)}function f(A,e){if("function"!=typeof e&&null!==e)throw new TypeError("Super expression must either be null or a function");A.prototype=Object.create(e&&e.prototype,{constructor:{value:A,writable:!0,configurable:!0}}),Object.defineProperty(A,"prototype",{writable:!1}),e&&m(A,e)}function d(A){var e="function"==typeof Map?new Map:void 0;return d=function(A){if(null===A||!function(A){try{return-1!==Function.toString.call(A).indexOf("[native code]")}catch(e){return"function"==typeof A}}(A))return A;if("function"!=typeof A)throw new TypeError("Super expression must either be null or a function");if(void 0!==e){if(e.has(A))return e.get(A);e.set(A,t)}function t(){return function(A,e,t){if(p())return Reflect.construct.apply(null,arguments);var n=[null];n.push.apply(n,e);var r=new(A.bind.apply(A,n));return t&&m(r,t.prototype),r}(A,arguments,h(this).constructor)}return t.prototype=Object.create(A.prototype,{constructor:{value:t,enumerable:!1,writable:!0,configurable:!0}}),m(t,A)},d(A)}function p(){try{var A=!Boolean.prototype.valueOf.call(Reflect.construct(Boolean,[],(function(){})))}catch(A){}return(p=function(){return!!A})()}function m(A,e){return m=Object.setPrototypeOf?Object.setPrototypeOf.bind():function(A,e){return A.__proto__=e,A},m(A,e)}function h(A){return h=Object.setPrototypeOf?Object.getPrototypeOf.bind():function(A){return A.__proto__||Object.getPrototypeOf(A)},h(A)
/**!
 * @license Media-Card-Web-Component - A web component that allows you to easily fetch / show off details from 
   your favorite anime, movies, TV shows, songs & books!
 * VERSION: 2.0.1
 * LICENSED UNDER MIT LICENSE
 * MORE INFO CAN BE FOUND AT https://github.com/MarketingPipeline/Media-Card-Web-Component/
*/}var v=new CSSStyleSheet;v.replaceSync("/*compress*/\n  @import url('https://fonts.googleapis.com/css?family=Montserrat:300,400,700,800');\n  :host { \n    --background: linear-gradient(270deg, #cfcfcf, #8b8b8b);\n    --background-size: 200% 100%;\n    --animation: 1.5s shine linear infinite;\n  }\n  * {\n    box-sizing: border-box;\n    margin: 0;\n  }\n  .media_card {\n    font-family: 'Montserrat', helvetica, arial, sans-serif;\n    font-size: 14px;\n    font-weight: 400;\n    position: relative;\n    display: block;\n    margin: 80px auto;\n    overflow: hidden;\n    border-radius: 10px;\n    transition: all 0.4s;\n    box-shadow: 0px 0px 120px -25px rgba(0, 0, 0, 0.5);\n    text-align: left;  \n    height: fit-content;\n    max-width: 800px;\n  }\n  @media (pointer:none), (pointer:coarse) {\n    .media_card {\n      min-height: 303px;\n    }\n  }\n  .info_section {\n    position: relative;\n    background-blend-mode: multiply;\n    height: 100%;\n    width: 100%;\n    z-index: 2;\n    border-radius: 10px;\n    display: flex;\n    flex-direction: column;\n  }\n  .media_header {\n    position: relative;\n    padding: 25px;\n    height: 40%;\n    display: flex;\n    flex-direction: row;    \n  }\n  h1 {\n    color: black;\n    font-weight: 400;\n  }\n  h4 {\n    color: #555;\n    font-weight: 400;\n  }\n  .minutes {\n    display: inline-block;\n    margin: .5rem 0;\n    color: #555;\n    padding: 0.25rem 0.5rem;\n    border-radius: 5px;\n    border: 1px solid rgba(0, 0, 0, 0.05);\n  }\n  .show-minutes {\n    display: inline-block;\n    color: #959595;\n    margin-top: .5rem;\n  }\n  .minutes ~ .show-minutes {\n    margin-top: unset;\n  }\n  .locandina {\n    position: relative;\n    float: left;\n    margin-right: 20px;\n    height: 120px;\n    box-shadow: 0 0 20px -10px rgba(0, 0, 0, 0.5);\n  }\n  .locandina-holding {\n    margin-right: 20px;\n    background: var(--background);\n    background-size: var(--background-size);\n    animation: var(--animation);\n    width: 80px;\n    height: 120px;\n    display: none;\n  }\n  .media_desc {\n    padding: 25px;\n    height: 50%;\n  }\n  .text {\n    color: #545454;\n    display: block;\n    font-size: 0.86rem;\n  }\n  \n  \n  .text:after {\n    content: \"\";\n    position: absolute;\n    bottom: 0;\n    right: 50%;\n    width: 100%;\n    height: 40px;\n    border-bottom: 8px solid #fff;\n    background: linear-gradient(to top, white, rgba(255, 255, 255, 0));\n}\n  \n  .blur_back {\n    position: absolute;\n    top: 0;\n    height: 100%;\n    right: 0;\n    width: 100%;\n    border-radius: 11px;\n    background-repeat: no-repeat;\n    background: url(\"data:image/jpeg;base64,\");\n    \n  }\n  .collection-name,\n  .primary-genre-name {\n    display: none;\n  }\n  @media screen and (min-width: 768px) {\n    .media_card {\n      min-height: 350px;\n    }\n    h1 {\n      width: 174%;\n    }\n    .media_header {\n      max-width: 50%;\n    }\n    .info_section {\n      height: 100%;\n      position: absolute;\n    }\n    .media_desc {\n      width: 50%;\n    }\n    .info_section {\n      background: linear-gradient(to right, #e5e6e6 50%, transparent 100%);\n    }\n    .blur_back {\n      width: 100%;\n      background-size: cover;\n      background-position: center center;\n      width: 80%;\n      left: 33%;\n    }\n  }\n  @media screen and (max-width: 768px) {\n    .text:after {\n      right: 0% !important;\n    }\n    .media_card .blur_back{\n      left:0%;\n    }\n    .media_card {\n      width: 95%;\n      margin: 70px auto;\n      min-height: 240px;\n    }\n    .blur_back {\n      width: 100%;\n      background-size: cover;\n      background-position: center top;\n      left: unset;\n    }\n    .media_header {\n      width: 100%;\n      margin-top: 85px;\n    }\n    .media_desc {\n      width: 100%;\n    }\n    .info_section {\n      background: linear-gradient(to top, #e5e6e6 50%, transparent 100%);\n      display: inline-grid;\n      height: unset;\n    }\n  }\n  .skeleton {\n    position: relative;\n  }\n  .skeleton .locandina,\n  .skeleton .minutes{\n    display: none;\n  }\n  .skeleton .locandina-holding {\n    display: block;\n  }\n  .skeleton h1 {\n    position: absolute;\n    left: 125px;\n    height: 32px;\n    width: 200px;\n    background: var(--background);\n    background-size: var(--background-size);\n    animation: var(--animation);\n  }\n  .skeleton h4 {\n    position: absolute;\n    left: 125px;\n    top: 60px;\n    width: 60px;\n    height: 16px;\n    background: var(--background);\n    background-size: var(--background-size);\n    animation: var(--animation);\n  }\n  @keyframes shine {\n    to {\n      background-position-x: -200%;\n    }\n  }\n/*endcompress*/");var b=new CSSStyleSheet;b.replaceSync('/*compress*/\n  h1 {\n    color: white;\n  }\n  h4 {\n    color: #9ac7fa;\n  }\n  .minutes {\n    color: #fff;\n    border: 1px solid rgba(255, 255, 255, 0.13);\n  }\n  .show-minutes {\n    color: #cee4fd;\n  }\n  .text {\n    color: #cfd6e1;\n  }\n   .text:after {\n    content: "";\n    position: absolute;\n    bottom: 0;\n    right: 50%;\n    width: 100%;\n    height: 40px;\n    border-bottom: 8px solid black;\n    background: linear-gradient(to top, black, rgba(255, 255, 255, 0));\n}\n  \n  .collection-name,\n  .primary-genre-name {\n    color: #cfd6e1;\n  }\n  @media screen and (min-width: 768px) {\n    .text:after {\n      right: 0% !important;\n      background: linear-gradient(to top, black, rgba(255, 255, 255, 0));\n    }\n    .info_section {\n      background: linear-gradient(to right, #0d0d0c 50%, transparent 100%);\n    }\n  }\n  @media screen and (max-width: 768px) {\n    .text:after {\n      right:0%;\n      background: linear-gradient(to top, black, rgba(255, 255, 255, 0));\n    }\n    .info_section {\n      background: linear-gradient(to top, #141413 50%, transparent 100%);\n    }\n  }\n/*endcompress*/');var y=new CSSStyleSheet;y.replaceSync("/*compress*/\n  .media_desc {\n    display: none;\n  }\n  .collection-name, \n  .primary-genre-name {\n    display: block;\n  }\n  @media screen and (min-width: 768px) {\n    .media_card {\n      min-height: 170px;\n    }\n  }\n  @media screen and (max-width: 768px) {\n    .media_card {\n      min-height: 240px;\n    }\n  }\n/*endcompress*/");var x=function(A){function e(){var A,t,n,r;!function(A,e){if(!(A instanceof e))throw new TypeError("Cannot call a class as a function")}(this,e),t=this,n=h(n=e),(A=u(t,p()?Reflect.construct(n,r||[],h(t).constructor):n.apply(t,r))).shadow=A.attachShadow({mode:"open"});var o=[v];if("light"!==A.theme&&o.push(b),"song"!==A.type&&"book"!==A.type||o.push(y),A.shadow.adoptedStyleSheets=o,A.createCard(),A.card=A.shadow.querySelector(".media_card"),A.locandina=A.shadow.querySelector(".locandina"),A.locandinaHolding=A.shadow.querySelector(".locandina-holding"),A.h1=A.shadow.querySelector("h1"),A.h4=A.shadow.querySelector("h4"),A.minutes=A.shadow.querySelector(".minutes"),A.showMinutes=A.shadow.querySelector(".show-minutes"),A.text=A.shadow.querySelector(".text"),A.blurBack=A.shadow.querySelector(".blur_back"),A.collectionName=A.shadow.querySelector(".collection-name"),A.primaryGenreName=A.shadow.querySelector(".primary-genre-name"),A.type&&"tv"!==A.type&&"book"!==A.type&&"song"!==A.type&&"film"!==A.type&&"episode"!==A.type)return A.populateError({status_message:"Please provide a valid type attribute."}),u(A);if(!A.name)return A.populateError({status_message:"Please provide an ".concat(A.type," name.")}),u(A);if("undefined"==typeof TheMovieDB_APIKey&&("tv"===A.type||"film"===A.type))return A.populateError({status_message:"Please provide an API Key"}),u(A);if(/\s\([0-9]{4}\)$/.test(A.name)){var a=s(/\s\(([0-9]{4})\)$/,{year:1}).exec(A.name);A.year=a.groups.year,A.mediaName=A.name.replace(/\s\([0-9]{4}\)$/,"")}else A.mediaName=A.name;if("film"!==A.type&&"tv"!==A.type||(A.endPoint="https://api.themoviedb.org/3/search/".concat("film"===A.type?"movie":"tv","?api_key=").concat(TheMovieDB_APIKey,"&language=en-US&query=").concat(A.mediaName),A.year&&(A.endPoint+="&".concat("film"===A.type?"year":"first_air_date_year","=").concat(A.year))),"episode"===A.type)if(A.episode&&A.season)A.endPoint="https://api.themoviedb.org/3/search/tv?api_key=".concat(TheMovieDB_APIKey,"&language=en-US&query=").concat(A.mediaName);else{if(!A.episode)return A.populateError({status_message:"Please provide an ".concat(A.type," number.")}),u(A);if(!A.season)return A.populateError({status_message:"Please provide an TV season number."}),u(A)}return"song"===A.type&&(A.endPoint="https://search-itunes.vercel.app?term=".concat(A.name,"&entity=song")),"book"===A.type&&(A.endPoint="https://openlibrary.org/search.json?title=".concat(A.name,"&limit=1"),A.author&&(A.endPoint="https://openlibrary.org/search.json?title=".concat(A.name,"&limit=1&author=").concat(A.author))),A.getDetails(),A}return f(e,d(HTMLElement)),t=e,n=[{key:"populateCardExtras",value:function(A){this.extraData=A,"film"===this.type&&this.extraData.runtime?this.minutes.innerText="".concat(this.extraData.runtime," mins"):this.minutes.remove(),"episode"!=this.type&&(this.showMinutes.innerText=this.extraData.genres.map((function(A){return A.name})).join(", ")),"episode"===this.type&&(this.showMinutes.innerText="Season ".concat(this.extraData.season_number),this.text.innerText=this.extraData.overview,this.h1.innerText=this.extraData.name,this.h4.innerText="Episode #".concat(this.extraData.episode_number),null==this.extraData.still_path?this.locandina.src="https://www.movienewz.com/img/films/poster-holder.jpg":this.locandina.src="https://image.tmdb.org/t/p/w500".concat(this.extraData.still_path))}},{key:"populateError",value:(m=i(o().mark((function A(e){return o().wrap((function(A){for(;;)switch(A.prev=A.next){case 0:this.card.classList.remove("skeleton"),this.h1.innerText="Error",this.locandinaHolding.style.display="block",this.locandina.remove(),this.minutes.remove(),e.errors?this.h4.innerText=e.errors.join(", "):this.h4.innerText=e.status_message;case 6:case"end":return A.stop()}}),A,this)}))),function(A){return m.apply(this,arguments)})},{key:"emptyResults",value:function(A){return void 0!==A.total_results&&0===A.total_results||void 0!==A.resultCount&&0===A.resultCount||0===A.numFound}},{key:"populateCard",value:function(A){this.card.classList.remove("skeleton"),this.emptyResults(A)?this.populateError({status_message:"Unable to find media"}):("tv"!==this.type&&"film"!==this.type&&"episode"!==this.type||(this.data=A.results[0],"episode"===this.type?this.extraEndPoint="https://api.themoviedb.org/3/tv/".concat(this.data.id,"/season/").concat(this.season,"/episode/").concat(this.episode,"?api_key=").concat(TheMovieDB_APIKey):this.extraEndPoint="https://api.themoviedb.org/3/".concat("film"===this.type?"movie":"tv","/").concat(this.data.id,"?api_key=").concat(TheMovieDB_APIKey),this.getExtraDetails(),null!==A.results[0].backdrop_path&&(this.blurBack.style.backgroundImage="url('https://image.tmdb.org/t/p/w500".concat(A.results[0].backdrop_path,"')")),this.h1.innerText="film"===this.type?this.data.original_title:this.data.original_name,this.h4.innerText="film"===this.type?null!=this.data.release_date?this.data.release_date.replace(/\-[0-9]{2}/g,""):"":null!=this.data.first_air_date?this.data.first_air_date.replace(/\-[0-9]{2}/g,""):"",this.text.innerText=this.data.overview,null==this.data.poster_path?this.locandina.src="https://www.movienewz.com/img/films/poster-holder.jpg":this.locandina.src="https://image.tmdb.org/t/p/w500".concat(this.data.poster_path)),"song"===this.type&&(this.data=A.results[0],this.minutes.remove(),this.showMinutes.remove(),this.h1.innerText=this.data.trackName,this.h4.innerText=this.data.artistName,this.locandina.src=this.data.artworkUrl100,this.blurBack.style.background='url("'.concat(this.data.artworkUrl100,'")'),this.collectionName.innerHTML=this.data.collectionName,this.primaryGenreName.innerHTML=this.data.primaryGenreName),"book"===this.type&&(this.data=A.docs[0],this.h1.innerText=this.data.title,this.h4.innerText=this.data.author_name.join(", "),this.data.cover_i?(this.locandina.src="https://covers.openlibrary.org/b/id/".concat(this.data.cover_i,"-M.jpg"),this.blurBack.style.background='url("https://covers.openlibrary.org/b/id/'.concat(this.data.cover_i,'-S.jpg")')):(this.locandina.remove(),this.locandinaHolding.remove()),this.data.number_of_pages_median?this.minutes.innerText="".concat(this.data.number_of_pages_median," pages"):(this.minutes.remove(),this.showMinutes.remove())))}},{key:"createCard",value:function(){var A=document.createElement("div");A.classList.add("media_card","skeleton");var e=document.createElement("div");e.classList.add("info_section");var t=document.createElement("div");t.classList.add("media_header");var n=document.createElement("div");n.classList.add("locandina-holding");var r=document.createElement("img");r.classList.add("locandina"),r.setAttribute("src","");var o=document.createElement("div");o.classList.add("details");var a=document.createElement("h1"),i=document.createElement("h4"),s=document.createElement("span");s.classList.add("minutes");var c=document.createElement("p");c.classList.add("show-minutes");var l=document.createElement("p");l.classList.add("collection-name");var u=document.createElement("p");u.classList.add("primary-genre-name"),o.append(a,i,s,c,l,u),t.append(n,r,o);var f=document.createElement("div");f.classList.add("media_desc");var d=document.createElement("p");d.classList.add("text"),f.appendChild(d),e.append(t,f);var p=document.createElement("div");p.classList.add("blur_back"),A.append(e,p),this.shadowRoot.append(A)}},{key:"getDetails",value:(l=i(o().mark((function A(){var e,t,n;return o().wrap((function(A){for(;;)switch(A.prev=A.next){case 0:return A.prev=0,A.next=3,fetch(this.endPoint,{mode:"cors"});case 3:if(!(e=A.sent).ok){A.next=11;break}return A.next=7,e.json();case 7:t=A.sent,this.populateCard(t),A.next=15;break;case 11:return A.next=13,e.json();case 13:n=A.sent,this.populateError(n);case 15:A.next=20;break;case 17:A.prev=17,A.t0=A.catch(0),this.populateError({status_message:"There was in issue with fetching the media details"});case 20:case"end":return A.stop()}}),A,this,[[0,17]])}))),function(){return l.apply(this,arguments)})},{key:"getExtraDetails",value:(a=i(o().mark((function A(){var e,t,n;return o().wrap((function(A){for(;;)switch(A.prev=A.next){case 0:return A.prev=0,A.next=3,fetch(this.extraEndPoint,{mode:"cors"});case 3:if(!(e=A.sent).ok){A.next=11;break}return A.next=7,e.json();case 7:t=A.sent,this.populateCardExtras(t),A.next=15;break;case 11:return A.next=13,e.json();case 13:n=A.sent,this.populateError(n);case 15:A.next=20;break;case 17:A.prev=17,A.t0=A.catch(0),this.populateError({status_message:"There was in issue with fetching the extra details for the media"});case 20:case"end":return A.stop()}}),A,this,[[0,17]])}))),function(){return a.apply(this,arguments)})},{key:"name",get:function(){return this.getAttribute("name")}},{key:"season",get:function(){return this.getAttribute("season")}},{key:"episode",get:function(){return this.getAttribute("episode")}},{key:"author",get:function(){return this.getAttribute("author")}},{key:"theme",get:function(){return this.getAttribute("theme")||"light"}},{key:"type",get:function(){return this.getAttribute("type")?this.getAttribute("type").toLowerCase():"film"}}],r=[{key:"observedAttributes",get:function(){return["name","author","theme","type","episode","season"]}}],n&&c(t.prototype,n),r&&c(t,r),Object.defineProperty(t,"prototype",{writable:!1}),t;var t,n,r,a,l,m}();window.customElements.define("media-card",x);